...
module Maestrano
  class SSOTest < Test::Unit::TestCase
    ...
    context 'with preset' do
      ...
      should "return the right init_url" do
        assert Maestrano::SSO[@preset].init_url == "http://localhost:3000/maestrano/auth/saml/init"
      end
    
      should "return the right consume_url" do
        assert Maestrano::SSO[@preset].consume_url == "http://localhost:3000/maestrano/auth/saml/consume"
      end
    
      should "return the right logout_url" do
        assert Maestrano::SSO[@preset].logout_url == "https://maestrano.com/app_logout"
      end
    
      should "return the right unauthorized_url" do
        assert Maestrano::SSO[@preset].unauthorized_url == "https://maestrano.com/app_access_unauthorized"
      end
    
      should "return the right idp_url" do
        assert Maestrano::SSO[@preset].idp_url == "https://maestrano.com/api/v1/auth/saml"
      end
    
      should "return the right session_check_url" do
        assert Maestrano::SSO[@preset].session_check_url('usr-1','f9ds8fdg7f89') == "https://maestrano.com/api/v1/auth/saml/usr-1?session=f9ds8fdg7f89"
      end
    
      should "return the right enabled parameter" do
        assert Maestrano::SSO[@preset].enabled? == !!Maestrano.param('sso.enabled')
      end
